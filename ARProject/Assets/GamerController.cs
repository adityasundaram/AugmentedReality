        ...
        if(!popupCanvas.enabled)
        {
            ...
            if (characterObjects != null && characterObjects.Capacity > 1 && characterObjects.Capacity > 1)
            {
                ...
                if (h != 0 || v != 0)
                {
                    ...
                    Vector3 pos = currentPlayerObject.charObject.transform.position + target;
                    currentPlayerObject.charObject.transform.position = Vector3.Lerp(currentPlayerObject.charObject.transform.position, pos, speed);
